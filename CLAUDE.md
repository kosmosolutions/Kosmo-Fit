...
- **Goal-weight ETA under the weight trend chart** — new `src/lib/goalEta.ts` (`projectGoalEta`) projects when the user reaches their goal weight. From ≥3 logged weights spanning ≥7 days (30-day window), it fits a least-squares trend and returns a `projected` ETA — anchored at the LAST LOG date/weight so the estimate only moves when a new weight is logged (going quiet freezes it, per user request). Fallbacks: `planned` (onboarding weeks_to_goal pace, anchored today) until enough logs exist; `stalled` when the trend is flat/reversing/>2yr out; `reached`; `none` (no goal). Rendered as a `GoalEtaFooter` in `WeightTrendChart` (overview + profile) with an ahead/on-pace/behind chip vs `stats.weeklyLoss` (±15% band). Chart gained `plannedWeeklyLoss` + `planHref` props; in the planned/stalled states the chip is an "Adjust plan" link (overview passes `/profile`) and renders NOTHING without `planHref` (profile), while pace verdicts stay read-only labels. Profile UX: "Save profile" routes back to `/overview`; the Timeframe section shows a live `weeks · lb/wk` line with the aggressive warning; `calcStats.aggressive` also triggers above 1% of body weight per week (old absolute-only >2 lb/wk rule never fired for lighter users).
